...
class IntegrationBase
{
  public:
    ...
    void push_back(double dt, const Eigen::Vector3d &acc, const Eigen::Vector3d &gyr)//加入buffer
    {
        dt_buf.push_back(dt);
        acc_buf.push_back(acc);
        gyr_buf.push_back(gyr);
        propagate(dt, acc, gyr);//拿imu数据进去预积分一波
    }
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        if(update_jacobian)//雅克比矩阵和协方差矩阵的传递
        {
    ...
    {
        dt = _dt;
        acc_1 = _acc_1;//当前加速度计测量值
        gyr_1 = _gyr_1;//当前陀螺仪测量值
        Vector3d result_delta_p;//暂时的p变化量+之前累计的p变化量
        Quaterniond result_delta_q;//暂时的q变化量+之前累计的q变化量
        Vector3d result_delta_v;//暂时的v变化量+之前累计的v变化量
        ...
        midPointIntegration(_dt, acc_0, gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v,
                            ...
                            result_linearized_ba, result_linearized_bg, 1);//实现了两个功能1.中值积分求result_delta_p,result_delta_q, result_delta_v,result_linearized_ba, result_linearized_bg
        //2.传递雅克比矩阵和误差协方差矩阵
        //checkJacobian(_dt, acc_0, gyr_0, acc_1, gyr_1, delta_p, delta_q, delta_v,
        //                    linearized_ba, linearized_bg);
        delta_p = result_delta_p;//累计的变化量，总的p
        delta_q = result_delta_q;//累计的变化量，总的q
        delta_v = result_delta_v;//累计的变化量，总的v
        linearized_ba = result_linearized_ba;
        linearized_bg = result_linearized_bg;
        delta_q.normalize();
        sum_dt += dt;
        acc_0 = acc_1;//上一帧加速度计测量值
        gyr_0 = gyr_1;//上一帧陀螺仪测量值
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        ///jacobian在midPointIntegration()里面计算并传递，是一个15×15的matrix，变量顺序为[p,q,v,ba,bg]对应0,3,6,9,12
        ...
        ///IMU预积分的结果,消除掉acc bias和gyro bias的影响,对应论文中公式（12），根据db修正预积分的值
        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);
        Eigen::Vector3d corrected_delta_v = delta_v + dv_dba * dba + dv_dbg * dbg;
        Eigen::Vector3d corrected_delta_p = delta_p + dp_dba * dba + dp_dbg * dbg;
        ///IMU项residual计算,输入参数是状态的估计值, 上面correct_delta_*是预积分值, 二者作差得到residual
