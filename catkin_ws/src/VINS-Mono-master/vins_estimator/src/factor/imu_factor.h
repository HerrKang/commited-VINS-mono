...
//delta_q = Qi.inverse() * Qj;
        ...
        Eigen::Map<Eigen::Matrix<double, 15, 1>> residual(residuals);//Eigen::Map相当于引用
        residual = pre_integration->evaluate(Pi, Qi, Vi, Bai, Bgi,
                                            Pj, Qj, Vj, Baj, Bgj);

        Eigen::Matrix<double, 15, 15> sqrt_info = Eigen::LLT<Eigen::Matrix<double, 15, 15>>(pre_integration->covariance.inverse()).matrixL().transpose();//P^-1的LT
        //sqrt_info.setIdentity();
        residual = sqrt_info * residual;//L.T*e,最终优化的是马氏距离e.T*L*L.T*e,这里相当于用L.T*e代替e
        ...
        if (jacobians)
        {
            ...
            if (pre_integration->jacobian.maxCoeff() > 1e8 || pre_integration->jacobian.minCoeff() < -1e8)//检查jacobian里的最大最小元素的大小
            {
...
///                ROS_BREAK();
            ...
            if (jacobians[0])
            {
                Eigen::Map<Eigen::Matrix<double, 15, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);//jacobian_pose_i是对jacobians[0]的引用
