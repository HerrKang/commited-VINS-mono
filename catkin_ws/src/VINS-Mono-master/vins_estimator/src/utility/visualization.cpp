...
void printStatistics(const Estimator &estimator, double t)
{
    if (estimator.solver_flag != Estimator::SolverFlag::NON_LINEAR)
        return;
    printf("position: %f, %f, %f\r", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z());//print窗口最后一帧的位姿
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        {
            cv::FileStorage fs(EX_CALIB_RESULT_PATH, cv::FileStorage::WRITE);//用于存储外参的估计结果
...
//!输出“odometry”,"path","relocalization_path"
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        foutC.close();//result每一行的数据：p,q,v
    }
}
//!输出"key_poses"
void pubKeyPoses(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        correct_pose = estimator.key_poses[i];//key_poses里面装的是Ps
...
//!输出"camera_pose","camera_pose_visual"
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        cameraposevisual.reset();//搁置pose_graph_node.cpp
        cameraposevisual.add_pose(P, R);
        cameraposevisual.publish_by(pub_camera_pose_visual, odometry.header);//怀疑是在画那个相机的框框
    }
}

//!输出"point_cloud"(header+特征点3D坐标),"history_cloud"(边缘化的点)
...
//!发布"keyframe_pose","keyframe_point"
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            {//!起始帧小于WINDOW_SIZE - 2&&末帧大于等于WINDOW_SIZE - 2&&是关键帧,对于pub的keyframe的特征点还是有一定的筛选
