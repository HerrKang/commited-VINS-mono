...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    if (sequence_cnt != cur_kf->sequence)//!一直到sequence_cnt=cur_kf->sequence
    {
        sequence_cnt++;
        sequence_loop.push_back(0);
        w_t_vio = Eigen::Vector3d(0, 0, 0);
	...
	if (loop_index != -1)//找到回环
	{
        ...
        if (cur_kf->findConnection(old_kf))
        {
            ...
            Matrix3d shift_r;
            Vector3d shift_t;
            //!shift between (old pose+relative pose) and (current pose)
            shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
            shift_r = Utility::ypr2R(Vector3d(shift_yaw, 0, 0));
            shift_t = w_P_cur - w_R_cur * vio_R_cur.transpose() * vio_P_cur; 
            //!shift vio pose of whole sequence to the world frame 似乎是以老帧的world frame为基准
    ...
    if (SHOW_S_EDGE)//!把当前帧和同sequence的关键帧连起来,不进入该if
    {
    ...
    if (SHOW_L_EDGE)//!把当前帧和同回环帧连起来,进入该if
    {
        if (cur_kf->has_loop)
        {
            //printf("has loop \n");
            ...
            if(cur_kf->sequence > 0)
            {
                ...
                posegraph_visualization->add_loopedge(P0, connected_P + Vector3d(VISUALIZATION_SHIFT_X, VISUALIZATION_SHIFT_Y, 0));//in launch file =0
            }
...
void PoseGraph::loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    global_index++;
    int loop_index = -1;
    if (flag_detect_loop)//新keyframe
       loop_index = detectLoop(cur_kf, cur_kf->index);//最小的回环帧的ID
    else//回环keyframe
    {
        addKeyFrameIntoVoc(cur_kf);
    }
    if (loop_index != -1)//新keyframe
    {
        printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
        KeyFrame* old_kf = getKeyFrame(loop_index);
        if (cur_kf->findConnection(old_kf))
    ...
    //draw local connection把当前帧和所有keyframe连在一起
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        ...
        cv::resize(keyframe->image, compressed_image, cv::Size(376, 240));//compressed_image处理过的Keyframe
    ...
    QueryResults ret;
    TicToc t_query;
    db.query(keyframe->brief_descriptors, ret, 4, frame_index - 50);//计算数据库里面的图像与传入的Keyframe的相似度 ,frame_index - 50是max_id,表示从50帧之前找
    ...
    if (DEBUG_IMAGE)
    {
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            ...
            cv::hconcat(loop_result, tmp_image, loop_result);//loop_result=[loop_result tmp_image]
        }
    }
...
*/
    if (find_loop && frame_index > 50)
    {
        ...
        return min_index;//最小的回环帧的ID，满足score>0.015
    }
    else
        return -1;

}
//!构造compressed_image&加入数据库for visualization
...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        Vector3d PG_tmp_T = (*it)->T_w_i;//vio_R_w_i和R_w_i有啥区别？？？不懂
...
void PoseGraph::loadPoseGraph()
{
    ...
    double PG_Qw, PG_Qx, PG_Qy, PG_Qz;//VIO的R,T与PG(pose graph)的R,T，世界系一个是老的世界系一个是当前的新的世界系
    ...
    while (fscanf(pFile,"%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d", &index, &time_stamp, 
                                    ...
                                    &keypoints_num) != EOF) //读文件中保存的pose_graph
    {
        /*
        ...
        */
        ...
        std::ifstream brief_file(brief_path, std::ios::binary);//是一个输入文件流
        string keypoints_path = POSE_GRAPH_SAVE_PATH + to_string(index) + "_keypoints.txt";
        FILE *keypoints_file;//文件指针
        ...
        KeyFrame* keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints, keypoints_norm, brief_descriptors);//老的keyframe
        loadKeyFrame(keyframe, 0);
        if (cnt % 20 == 0)
        {
            publish();
        }
        cnt++;//读一个pose_graph,cnt就++
    }
    fclose (pFile);
...
void PoseGraph::updateKeyFrameLoop(int index, Eigen::Matrix<double, 8, 1 > &_loop_info)
{
    ...
    if (abs(_loop_info(7)) < 30.0 && Vector3d(_loop_info(0), _loop_info(1), _loop_info(2)).norm() < 20.0)
    {
        if (FAST_RELOCALIZATION)//一般用不到……
        {
            ...
            w_P_cur = w_R_old * relative_t + w_P_old;
            w_R_cur = w_R_old * relative_q;//通过老Rt和相对Rt得到新Rt,注意vio_P_cur和vio_R_cur是通过新帧得到的，drift算的是这两者的差值
            double shift_yaw;
            Matrix3d shift_r;
            Vector3d shift_t; 
            shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();
