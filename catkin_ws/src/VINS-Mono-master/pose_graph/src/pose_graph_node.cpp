 ...
 * Input:IMAGE_TOPIC
 * 图像放到image_buf里面
 * if(detect unstable camera stream),then new_sequence()
 */
void image_callback(const sensor_msgs::ImageConstPtr &image_msg)
{
    //ROS_INFO("image_callback!");
    if(!LOOP_CLOSURE)//configuration、文件里面LOOP_CLOSURE=1才往下走
        return;
    m_buf.lock();
    ...
    if (last_image_time == -1)//初值
        last_image_time = image_msg->header.stamp.toSec();
    else if (image_msg->header.stamp.toSec() - last_image_time > 1.0 || image_msg->header.stamp.toSec() < last_image_time)//下一波图像(rosbag)
    {
...
/**
 * Input:"/vins_estimator/keyframe_point"
 * 把sliding window里面的特征点放到point_buf里
 */
...
/**
 * Input:"/vins_estimator/keyframe_pose",Ps,Vs,Rs
 * 把keyframe的位姿放到pose_buf里
 */
...
/**
 * Input:"/vins_estimator/odometry",Ps,Vs,Rs
 * use 4DOF optimization to correct the drift of Rt
 * pub_camera_pose_visual.publish(markerArray_msg)
 * pub_key_odometrys.publish(key_odometrys):10 red spheres(sphere list)
 * pub_vio_path.publish(no_loop_path):path with no loop
 */
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    vio_t = posegraph.w_r_vio * vio_t + posegraph.w_t_vio;//w_r_vio,w_t_vio是考虑到world frame的drift的修正因子
    vio_q = posegraph.w_r_vio *  vio_q;

    vio_t = posegraph.r_drift * vio_t + posegraph.t_drift;//4DOF优化校正的r,t的漂移
    vio_q = posegraph.r_drift * vio_q;
    ...
    if (odometry_buf.size() > 10)//10 elements in buffer
    {
        odometry_buf.pop();
    }
...
void process()
{
    ...
    while (true)
    {
        ...
        /// find out the messages with same time stamp获得时间戳尽量相同的image_msg,point_msg和pose_msg
        m_buf.lock();
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())//!三个buffer都非空
        {
            if (image_buf.front()->header.stamp.toSec() > pose_buf.front()->header.stamp.toSec())
            {//!丢掉pose_buf里面的时间戳比image_buf小的pose
                pose_buf.pop();
                printf("throw pose at beginning\n");
            }
            else if (image_buf.front()->header.stamp.toSec() > point_buf.front()->header.stamp.toSec())
            {//!丢掉point_buf里面的时间戳比image_buf小的pose
                point_buf.pop();
        ...
        if (pose_msg != NULL)
        {
            ...
            if (image_msg->encoding == "8UC1")
            {
                ...
                img.encoding = "mono8";
                ptr = cv_bridge::toCvCopy(img, sensor_msgs::image_encodings::MONO8);//sensor_msgs::Image变成cv Mat图像
            }
            else
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                   point_3d, point_2d_uv, point_2d_normal, point_id, sequence);//新关键帧
                m_process.lock();
                start_flag = 1;
                posegraph.addKeyFrame(keyframe, 1);//新关键帧加入posegraph
                m_process.unlock();
                frame_index++;
                last_t = T;
            }
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n("~");
    posegraph.registerPub(n);//发布一些topic
    ...
    n.getParam("config_file", config_file);//从launch文件读取configuration
    ...
    if (LOOP_CLOSURE)
    {
        ...
        posegraph.loadVocabulary(vocabulary_file);//载入词典文件

        BRIEF_PATTERN_FILE = pkg_path + "/../support_files/brief_pattern.yml";//brief描述子文件路径
        ...
        if (LOAD_PREVIOUS_POSE_GRAPH)
        {
            ...
            m_process.lock();
            posegraph.loadPoseGraph();//载入以前保存的pose_graph
            m_process.unlock();
    ...
    pub_match_img = n.advertise<sensor_msgs::Image>("match_image", 1000);//回环帧
