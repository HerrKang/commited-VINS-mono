 ...
 * @input  window_descriptor [当前滑窗中的描述子，强角点对应的描述子？？？]
 * @input  descriptors_old   [闭环帧的描述子集合]
 * @input  keypoints_old     [闭环帧的关键点集合(强角点+Fast角点)]
 * @input  keypoints_old_norm[]
 * @output  best_match       [最好的匹配点(闭环帧中)]
 * @output  best_match_norm  [最好的匹配点归一化坐标]
 * @return                   [description]
 */
                            ...
                            cv::Point2f &best_match_norm)
{
    ...
    {
		//！计算当前滑窗中描述子与闭环帧描述子的距离，并计算最短汉明距离和闭环帧中的描述子编号
 ...
 * @output matched_2d_old      	[闭环帧中最佳匹配点的像素坐标]
 * @output matched_2d_old_norm 	[description]
 * @output status 				[description]
 * @input descriptors_old       [闭环帧的描述子]
 * @input keypoints_old         [闭环帧的关键点]
 * @input keypoints_old_norm    [description]
 */
void KeyFrame::searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_old,
                                ...
                                const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
    for(int i = 0; i < (int)window_brief_descriptors.size(); i++)//对于每一个当前滑窗中的特征
    {
        cv::Point2f pt(0.f, 0.f);
        cv::Point2f pt_norm(0.f, 0.f);
        if (searchInAera(window_brief_descriptors[i], descriptors_old, keypoints_old, keypoints_old_norm, pt, pt_norm))//计算闭环帧中最好的匹配描述子
          status.push_back(1);//status[i]=0表示没search到最佳匹配描述子，status[i]=1表示search到了最佳匹配描述子
        else
          status.push_back(0);
 ...
 * @param matched_2d_old_norm   [description]
 * @param matched_3d 			[description]
 * @param status				[匹配点做PNP的结果，i为匹配点编号，1表示为内点]
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    PnP_R_old = R_w_c_old * qic.transpose();//R_w_b
    PnP_T_old = T_w_c_old - PnP_R_old * tic;//T_w_b=T_w_c-R_w_b*t_b_c

}

/**
 * [KeyFrame::findConnection 计算闭环帧与当前帧的位姿关系和匹配关系]
 * @param  old_kf                [闭环帧]
 * @return                       [description]
 */
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	matched_2d_cur_norm = point_2d_norm;//相机坐标系归一化平面坐标
	matched_id = point_id;
	...
	#endif
	//printf("search by des\n");根据老帧的关键点和描述子在新帧中找匹配点的像素坐标matched_2d_old和归一化平面坐标matched_2d_old_norm
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            cv::hconcat(gap_image, old_img, gray_img);
	            cvtColor(gray_img, loop_match_img, CV_GRAY2RGB);
	            for(int i = 0; i< (int)matched_2d_cur.size(); i++)//当前帧的点
	            {
	                cv::Point2f cur_pt = matched_2d_cur[i];
	                cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 255, 0));
	            }
	            for(int i = 0; i< (int)matched_2d_old.size(); i++)//回环帧的点
	            {
	                cv::Point2f old_pt = matched_2d_old[i];
	                old_pt.x += (COL + gap);
	                cv::circle(loop_match_img, old_pt, 5, cv::Scalar(0, 255, 0));
	            }
	            for (int i = 0; i< (int)matched_2d_cur.size(); i++)//匹配点连起来
	            {
	                cv::Point2f old_pt = matched_2d_old[i];
	                old_pt.x += (COL + gap) ;
	            ...
	            */
	            if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	            {
	            	...
	            	*/
	    	    	...
	    	    	pub_match_img.publish(msg);//发布回环帧
	            }
	        }
	    #endif
	}
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    relative_q = PnP_R_old.transpose() * origin_vio_R;//b_old_origin
	    ...
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0)
	    {
	    	...
	    	if(FAST_RELOCALIZATION)
	    	{
			    ...
			    msg_match_points.header.stamp = ros::Time(time_stamp);//时间戳，msg_match_points.header.stamp，注意用的是当前匹配帧的时间戳
			    for (int i = 0; i < (int)matched_2d_old_norm.size(); i++)
			    {
		            geometry_msgs::Point32 p;
		            p.x = matched_2d_old_norm[i].x;//像素坐标uv
		            p.y = matched_2d_old_norm[i].y;
		            p.z = matched_id[i];//匹配特征点id
		            msg_match_points.points.push_back(p);//像素坐标+匹配特征点id，加入msg_match_points.points
			    }
			    ...
			    t_q_index.values.push_back(Q.z());//匹配点所在的帧的R,T
			    t_q_index.values.push_back(index);//当前帧的index
			    msg_match_points.channels.push_back(t_q_index);//加入msg_match_points.channels
